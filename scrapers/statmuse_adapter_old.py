...
def get_team_stats_for_matchup(
    ...
    if not away_slug or not home_slug:
        logger.error(f"Could not find slugs for teams: {away_team} ({away_slug}), {home_team} ({home_slug})")
        return None, None

    # Fetch team stats and splits
    away_data = _get_team_complete_stats(away_slug, season, headless)
    home_data = _get_team_complete_stats(home_slug, season, headless)
...
def _get_team_complete_stats(team_slug: str, season: str, headless: bool) -> Optional[Dict]:
    """
    ...
    """
    cache_key = f"{team_slug}_{season}"

    # Check cache
    if cache_key in _team_stats_cache:
        logger.debug(f"Using cached stats for {team_slug}")
    ...
    # Scrape team splits
    splits = scrape_team_splits(team_slug, season, headless)
...
def get_player_season_average(
    ...
    if not team_slug:
        logger.error(f"Could not find slug for team: {team_name}")
        return None

    cache_key = f"{team_slug}_{season}_players"

    # Check cache
    if cache_key not in _player_stats_cache:
        # Scrape all players for this team
        players = scrape_player_stats(team_slug, season, headless)
