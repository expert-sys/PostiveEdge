"""
StatMuse Scraper V2 - Robust Edition with Correct Team IDs
==========================================================
Enhanced with retry logic, better error handling, and correct StatMuse URLs.
"""
...
def scrape_team_stats(team_name: str, season: str = "2025-26", headless: bool = True) -> Optional[TeamStats]:
    """
    Scrape team statistics from StatMuse (robust version with correct URLs).
    ...
    """
    try:
        url = get_team_stats_url(team_name)
    except ValueError as e:
        logger.error(f"Cannot get URL for team: {e}")
        return None

    logger.info(f"Scraping team stats: {team_name} ({season})")
    ...
    try:
        with sync_playwright() as p:
            ...
            # Initialize team stats (team_name passed as parameter)
...
def scrape_player_stats(team_name: str, season: str = "2025-26", headless: bool = True) -> List[PlayerStats]:
    """
    Scrape player statistics for a team from StatMuse (robust version with correct URLs).
    """
    try:
        url = get_team_stats_url(team_name)
    except ValueError as e:
    ...
    logger.info(f"Scraping player stats: {team_name} ({season})")
    ...
    try:
        with sync_playwright() as p:
            ...
            # Use team_name parameter directly (already in correct format)
            team_name_for_stats = team_name
...
def scrape_team_splits(team_name: str, season: str = "2025-26", headless: bool = True) -> List[TeamSplitStats]:
    """
    Scrape team split statistics from StatMuse (robust version with correct URLs).
    ...
    Returns:
        List of TeamSplitStats objects
    """
    try:
        url = get_team_splits_url(team_name)
    except ValueError as e:
        logger.error(f"Cannot get URL for team: {e}")
        return []

    logger.info(f"Scraping team splits: {team_name} ({season})")
    logger.info(f"URL: {url}")
